class ReferenceMonitor:
    ...
    def execute_query(self, subject_name, object_name):
        ...
        if self.integrity_levels[object_level] < self.integrity_levels[subject_level]:
            print(f'Access Denied: query {subject_name} {object_name}')
            return
    ...
    def execute_exchange(self, exchange_type, subject_name, object_name, amount):
        ...
        if self.integrity_levels[subject_level] < self.integrity_levels[object_level]:
            print(
                f'Access Denied: {exchange_type} {subject_name} {subject_name} {amount}')
            return
        ...
        if exchange_type == 'deposit':
            ...
            print('Access Granted:' +
                  f' {s.name} deposits {amount} to {o.name} ')
        else:
            o.balance -= amount
            print('Access Granted:' +
                  f' {s.name} withdraws {amount} to {o.name} ')
    ...
    def print_status(self, title):
        ...
        for key, subjects in self.subject_levels.items():
            for subject in subjects:
                print(
                    f"-> Name: '{subject.name}' Level: '{key}' Balance: '{subject.balance}'")
        print('----------------------------------------------------------------')
        print('                         Objects')
        for key, objects in self.object_levels.items():
            for obj in objects:
                print(
                    f"-> Name: '{obj.name}' Level: '{key}' Balance: '{obj.balance}'")
        print('----------------------------------------------------------------')
